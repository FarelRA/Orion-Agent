...
func (s *SyncService) SyncAllGroups(ctx context.Context) error {
	...
	for _, g := range groups {
		// Normalize JID
		g.JID = s.service.NormalizeJID(ctx, g.JID)
		g.OwnerJID = s.service.NormalizeJID(ctx, g.OwnerJID)
		g.NameSetBy = s.service.NormalizeJID(ctx, g.NameSetBy)
		g.TopicSetBy = s.service.NormalizeJID(ctx, g.TopicSetBy)
		...
		for _, p := range g.Participants {
			// Normalize JID
			p.JID = s.service.NormalizeJID(ctx, p.JID)
...
func (s *SyncService) SyncGroupInfo(ctx context.Context, jid types.JID) error {
	...
	// Normalize JID
	info.JID = s.service.NormalizeJID(ctx, info.JID)
	info.OwnerJID = s.service.NormalizeJID(ctx, info.OwnerJID)
	info.NameSetBy = s.service.NormalizeJID(ctx, info.NameSetBy)
	info.TopicSetBy = s.service.NormalizeJID(ctx, info.TopicSetBy)
...
func (s *SyncService) SyncAllGroupPictures(ctx context.Context) error {
	...
	for _, group := range groups {
		// Normalize JID
		group.JID = s.service.NormalizeJID(ctx, group.JID)
